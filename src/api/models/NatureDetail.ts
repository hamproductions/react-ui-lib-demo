...
export function instanceOfNatureDetail(value: object): value is NatureDetail {
  ...
  if (!("decreasedStat" in value) || value["decreasedStat"] === undefined)
    return false;
  if (!("increasedStat" in value) || value["increasedStat"] === undefined)
    return false;
  if (!("likesFlavor" in value) || value["likesFlavor"] === undefined)
    return false;
  if (!("hatesFlavor" in value) || value["hatesFlavor"] === undefined)
    return false;
  if (!("berries" in value) || value["berries"] === undefined) return false;
  if (
    !("pokeathlonStatChanges" in value) ||
    value["pokeathlonStatChanges"] === undefined
  )
    return false;
  if (
    !("moveBattleStylePreferences" in value) ||
    value["moveBattleStylePreferences"] === undefined
  )
    return false;
...
export function NatureDetailFromJSONTyped(
  json: any,
  ...
  return {
    ...
    pokeathlonStatChanges: (json["pokeathlon_stat_changes"] as Array<any>).map(
      NatureDetailPokeathlonStatChangesInnerFromJSON
    ),
    moveBattleStylePreferences: (
      json["move_battle_style_preferences"] as Array<any>
    ).map(NatureBattleStylePreferenceFromJSON),
    names: (json["names"] as Array<any>).map(NatureNameFromJSON),
  };
}

export function NatureDetailToJSON(
  value?: Omit<NatureDetail, "id"> | null
): any {
  if (value == null) {
    return value;
  }
  return {
    ...
    pokeathlon_stat_changes: (value["pokeathlonStatChanges"] as Array<any>).map(
      NatureDetailPokeathlonStatChangesInnerToJSON
    ),
    move_battle_style_preferences: (
      value["moveBattleStylePreferences"] as Array<any>
    ).map(NatureBattleStylePreferenceToJSON),
