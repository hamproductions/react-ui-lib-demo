...
import type { MoveDetailMeta } from "./MoveDetailMeta";
import {
  MoveDetailMetaFromJSON,
  MoveDetailMetaFromJSONTyped,
  MoveDetailMetaToJSON,
} from "./MoveDetailMeta";
...
export function instanceOfMoveDetail(value: object): value is MoveDetail {
  ...
  if (!("effectChance" in value) || value["effectChance"] === undefined)
    return false;
  if (!("contestCombos" in value) || value["contestCombos"] === undefined)
    return false;
  if (!("contestType" in value) || value["contestType"] === undefined)
    return false;
  if (!("contestEffect" in value) || value["contestEffect"] === undefined)
    return false;
  if (!("damageClass" in value) || value["damageClass"] === undefined)
    return false;
  if (!("effectEntries" in value) || value["effectEntries"] === undefined)
    return false;
  if (!("effectChanges" in value) || value["effectChanges"] === undefined)
    return false;
  if (!("generation" in value) || value["generation"] === undefined)
    return false;
  if (!("meta" in value) || value["meta"] === undefined) return false;
  if (!("names" in value) || value["names"] === undefined) return false;
  if (!("pastValues" in value) || value["pastValues"] === undefined)
    return false;
  if (!("statChanges" in value) || value["statChanges"] === undefined)
    return false;
  if (
    !("superContestEffect" in value) ||
    value["superContestEffect"] === undefined
  )
    return false;
  ...
  if (
    ...
    return false;
  if (!("learnedByPokemon" in value) || value["learnedByPokemon"] === undefined)
    return false;
  return true;
}
...
export function MoveDetailFromJSONTyped(
  json: any,
  ...
  return {
    ...
    effectChanges: (json["effect_changes"] as Array<any>).map(
      MoveDetailEffectChangesInnerFromJSON
    ),
    generation: GenerationSummaryFromJSON(json["generation"]),
    meta: MoveDetailMetaFromJSON(json["meta"]),
    names: (json["names"] as Array<any>).map(MoveNameFromJSON),
    pastValues: (json["past_values"] as Array<any>).map(MoveChangeFromJSON),
    statChanges: (json["stat_changes"] as Array<any>).map(
      MoveDetailStatChangesInnerFromJSON
    ),
    superContestEffect: SuperContestEffectSummaryFromJSON(
      json["super_contest_effect"]
    ),
    target: MoveTargetSummaryFromJSON(json["target"]),
    type: TypeSummaryFromJSON(json["type"]),
    machines: (json["machines"] as Array<any>).map(
      MoveDetailMachinesInnerFromJSON
    ),
    flavorTextEntries: (json["flavor_text_entries"] as Array<any>).map(
      MoveFlavorTextFromJSON
    ),
    learnedByPokemon: (json["learned_by_pokemon"] as Array<any>).map(
      AbilityDetailPokemonInnerPokemonFromJSON
    ),
  };
}

export function MoveDetailToJSON(value?: Omit<MoveDetail, "id"> | null): any {
  if (value == null) {
    return value;
  }
  return {
