...
export function instanceOfGenderDetail(value: object): value is GenderDetail {
  ...
  if (
    !("pokemonSpeciesDetails" in value) ||
    value["pokemonSpeciesDetails"] === undefined
  )
    return false;
  if (
    !("requiredForEvolution" in value) ||
    value["requiredForEvolution"] === undefined
  )
    return false;
  return true;
}
...
export function GenderDetailFromJSONTyped(
  json: any,
  ...
  return {
    id: json["id"],
    name: json["name"],
    pokemonSpeciesDetails: (json["pokemon_species_details"] as Array<any>).map(
      GenderDetailPokemonSpeciesDetailsInnerFromJSON
    ),
    requiredForEvolution: (json["required_for_evolution"] as Array<any>).map(
      AbilityDetailPokemonInnerPokemonFromJSON
    ),
  };
}

export function GenderDetailToJSON(
  value?: Omit<GenderDetail, "id"> | null
): any {
  if (value == null) {
    return value;
  }
  return {
    name: value["name"],
    pokemon_species_details: (value["pokemonSpeciesDetails"] as Array<any>).map(
      GenderDetailPokemonSpeciesDetailsInnerToJSON
    ),
    required_for_evolution: (value["requiredForEvolution"] as Array<any>).map(
      AbilityDetailPokemonInnerPokemonToJSON
    ),
  };
}
