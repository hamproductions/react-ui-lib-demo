 ...
 */
export function instanceOfLocationAreaDetail(
  value: object
): value is LocationAreaDetail {
  if (!("id" in value) || value["id"] === undefined) return false;
  if (!("name" in value) || value["name"] === undefined) return false;
  if (!("gameIndex" in value) || value["gameIndex"] === undefined) return false;
  if (
    !("encounterMethodRates" in value) ||
    value["encounterMethodRates"] === undefined
  )
    return false;
  if (!("location" in value) || value["location"] === undefined) return false;
  if (!("names" in value) || value["names"] === undefined) return false;
  if (
    !("pokemonEncounters" in value) ||
    value["pokemonEncounters"] === undefined
  )
    return false;
  return true;
}
...
export function LocationAreaDetailFromJSONTyped(
  json: any,
  ...
  return {
    ...
    encounterMethodRates: (json["encounter_method_rates"] as Array<any>).map(
      LocationAreaDetailEncounterMethodRatesInnerFromJSON
    ),
    location: LocationSummaryFromJSON(json["location"]),
    names: (json["names"] as Array<any>).map(LocationAreaNameFromJSON),
    pokemonEncounters: (json["pokemon_encounters"] as Array<any>).map(
      LocationAreaDetailPokemonEncountersInnerFromJSON
    ),
  };
}

export function LocationAreaDetailToJSON(
  value?: Omit<LocationAreaDetail, "id"> | null
): any {
  if (value == null) {
    return value;
  }
  return {
    ...
    encounter_method_rates: (value["encounterMethodRates"] as Array<any>).map(
      LocationAreaDetailEncounterMethodRatesInnerToJSON
    ),
    location: LocationSummaryToJSON(value["location"]),
    names: (value["names"] as Array<any>).map(LocationAreaNameToJSON),
    pokemon_encounters: (value["pokemonEncounters"] as Array<any>).map(
      LocationAreaDetailPokemonEncountersInnerToJSON
    ),
  };
}
